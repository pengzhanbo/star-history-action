...
| ---------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `action.yaml` | Runtime contract: composite action — `pnpm/action-setup`, `actions/cache@v4` (restores `node_modules`; key built from a `run`-step lockfile hash), `pnpm install --frozen-lockfile --prod`, then `node dist/index.js` with all seven inputs mapped to `INPUT_*` env vars |
