...
1. Composite `runs` in `action.yaml`: set up pnpm (version from the `packageManager` field) → `pnpm install --frozen-lockfile --prod` in the action directory → `node dist/index.js` with every input mapped to an `INPUT_*` env var (`@actions/core` reads them).
...
| ---------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `action.yaml` | Runtime contract: composite action — `pnpm/action-setup`, `pnpm install --frozen-lockfile --prod`, then `node dist/index.js` with all six inputs mapped to `INPUT_*` env vars |
