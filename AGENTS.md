...
2. **Entry** — `src/index.ts`: `parseInputs()` (`config.ts`, `repo` splits into a deduped `repos: string[]`) → fetch records + logo per repo in parallel (`services/api.ts`, each repo keeps its own request budget) → render one SVG per theme (`render.ts`, jsdom + `XYChart`, one dataset per repo) into `output-directory` under `GITHUB_WORKSPACE` (missing → hard error), rasterizing to PNG via **resvg** (`common/raster.ts` `rasterizeSvg`) when `output-format` is `png`/`both`; when `radar` is on, additionally fetch six metrics per repo (`services/radar.ts`) and render one radar SVG per repo (`charts/radar-svg.ts`) into the same directory → `commitAndPush()` (`git.ts`, identity `github-actions[bot]`; authenticated push via `GITHUB_REPOSITORY` on runners, `origin HEAD` locally; skipped entirely on `pull_request` events). All file writes go through `common/output.ts` `writeOutput`, which logs the workspace-relative path and returns it for the commit file list. Failures surface through `setFailed`.
3. **Fetch layer** — `src/services/api.ts`: `request()`, `getRepoStargazers()`, `getRepoStarRecords()`, `getRepoLogo()` against `GITHUB_API_URL` (default `api.github.com`; per-page pagination; token auth header; rate-limit constant `DEFAULT_MAX_REQUEST_AMOUNT = 15` in `covert.ts`). `request()` retries 5xx/network failures with exponential backoff (`RETRY_MAX_ATTEMPTS`, `RETRY_BASE_DELAY_MS`) and sleeps up to `MAX_RATE_LIMIT_WAIT_MS` for a near rate-limit reset (403/429 with `x-ratelimit-remaining: 0`), else fails fast with the reset time in the error. `getRepoStarRecords` is order-agnostic: it reads page 1's `Link` header for the page count (parsed via URLSearchParams on the `rel="last"` link, so GHES instances that reorder/omit query params keep working), detects whether page 1 is oldest-first or newest-first from the repo `created_at` distance (falling back to newest-first when `created_at` is missing/unparseable), and either counts every stargazer per day (full history) or samples boundary pages (±100 stars per point in sampled mode). Page 1 is fetched once for the `Link` probe and reused, never refetched. Radar metrics (`src/services/radar.ts`) reuse `request()` + `parseLastPage()`: stars/forks from `/repos/{repo}`, contributors/commits approximated as `lastPage * 100` from `per_page=1` list calls, closed issues from the search API's `total_count`; each is mapped to a 0–99 score via `percentileOf` (log10 scale, cap 99), and `new_stars` is derived from the already-fetched records (30-day window, no extra request).
4. **Rendering layer** — `src/charts/`, two families:
   ...
   - **Satori OG-card family** (`radar-svg.ts`, `card-landscape1.ts`): pure and import-free — radar returns a full standalone SVG **string** (base64-embedded as `<img>` in the card); card returns a plain `{ type, props: { children } }` element tree (private `h()` helper, satori/React-compatible) sized 1200×630. `renderRadarSvg` takes a `theme` (`light`/`dark` palettes) and inlines the xkcd font as a **woff2 subset** (`getSubsetFontUrl` over the axis labels + level numbers, with a full-ttf fallback), matching the history chart's subsetting.
5. **Output** — per theme, the serialized SVG (`jsdom` `outerHTML`) is written verbatim; when `output-format` is `png`/`both` the SVG bytes are additionally rasterized to a same-stem `.png` via **resvg** (`@resvg/resvg-js`, `common/raster.ts` `rasterizeSvg`), which loads the xkcd font explicitly from `assets/xkcd.ttf` (resolved from the action repo root) so the PNG text style matches the SVG — unlike librsvg (sharp's engine), which ignores the inlined `@font-face` and falls back to a system font. The SVG's `background:` style is parsed out and passed as resvg's `background` option (resvg ignores CSS background on the root element); transparent backgrounds pass no option. The resvg buffer is then re-encoded with **sharp palette quantization** (`compressionLevel: 9, palette: true`): chart palettes stay far below 256 colors, so it is lossless (pixel-identical) while shrinking files ~65%. Every write goes through `common/output.ts` `writeOutput` (write + workspace-relative log + commit-list path). `src/render.ts` is the jsdom bridge: it pins `clientWidth` to the `svg-width` input (jsdom has no layout) and passes `chartWidth` so a non-empty title logo lands on-canvas; only the XY-chart family is used today.
...
| `src/services/` | GitHub API client (`api.ts`) + request constants (`covert.ts`) + radar metric fetch/score mapping (`radar.ts`)                                                                                                                                                                                                                                     |
...
| `src/common/`   | Shared assets & output helpers: `colors.ts` (dataset hex palettes), `fonts.ts` (inline base64 xkcd woff), `font-subset.ts` (woff2 subsetting), `image-min.ts` (logo optimization), `raster.ts` (`rasterizeSvg` — resvg + sharp palette PNG encoding), `output.ts` (`writeOutput` — write + log + return relative path)                             |
