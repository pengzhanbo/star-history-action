...
2. **Entry** — `src/index.ts`: `parseInputs()` (`config.ts`, `repo` splits into a deduped `repos: string[]`) → fetch records + logo per repo in parallel (`services/api.ts`, each repo keeps its own request budget) → render one SVG per theme (`render.ts`, jsdom + `XYChart`, one dataset per repo) into `output-directory` under `GITHUB_WORKSPACE` (missing → hard error), rasterizing to PNG via **resvg** (`@resvg/resvg-js`, inlined `rasterizeSvg`) when `output-format` is `png`/`both`; when `radar` is on, additionally fetch six metrics per repo (`services/radar.ts`) and render one radar SVG per repo (`charts/radar-svg.ts`) into the same directory → `commitAndPush()` (`git.ts`, identity `github-actions[bot]`; authenticated push via `GITHUB_REPOSITORY` on runners, `origin HEAD` locally; skipped entirely on `pull_request` events). Failures surface through `setFailed`.
